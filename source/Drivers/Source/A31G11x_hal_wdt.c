...
HAL_Status_Type HAL_WDT_Init( WDT_CFG_Type* WDT_Config )
{
   ...
   // 2. immediately disable counter and reset
   WDT->CR = ( 0x5A69 << WDT_CR_WTIDKY_Pos )
           | ( 0x25  << WDT_CR_RSTEN_Pos )
           | ( 0x1A << WDT_CR_CNTEN_Pos );

   // 3. clear any pending status flags
   WDT->SR = 0x03;
   ...
   // 5. start counter WITHOUT reset (to load DR into counter)
   reg_val = WDT_Config->wdtClkDiv;
   WDT->CR = ( 0x5A69 << WDT_CR_WTIDKY_Pos )
           | ( 0x25 << WDT_CR_RSTEN_Pos )     // reset still disabled
           | reg_val;                          // CNTEN=0 (counter enabled)

   // 6. reload counter (now counter is running, reload works)
   WDT->CNTR = 0x6a;

   // 7. clear status again after reload
   WDT->SR = 0x03;

   // 8. now enable reset if requested
   if( WDT_Config->wdtResetEn == ENABLE )
   {
      WDT->CR = ( 0x5A69 << WDT_CR_WTIDKY_Pos ) | reg_val;  // RSTEN=0 (enabled), CNTEN=0 (enabled)
   }
